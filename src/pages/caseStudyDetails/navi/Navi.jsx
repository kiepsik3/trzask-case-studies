    ...
    return (
        <div className={cn("navi", pinned && "pinned", fixed && "fixed")}>
            <div className="navi-container">
                <Link to={`/pl/case-studies/${prevItem?.slug}`} className={cn("navi-link", prevItem?.slug === undefined && "disabled")} ref={(el) => {
                    el && fixed && el.style.setProperty('cursor', prevItem?.slug === undefined ? "default" : "pointer", 'important')
                }}><FaArrowLeft />Poprzedni</Link>
                <Link to="/pl/case-studies" className="navi-link" ref={(el) => {
                    el && fixed && el.style.setProperty('cursor', "pointer", 'important')
                }}>Lista</Link>
                <Link to={`/pl/case-studies/${nextItem?.slug}`} className={cn("navi-link", nextItem?.slug === undefined && "disabled")} ref={(el) => {
                    el && fixed && el.style.setProperty('cursor', nextItem?.slug === undefined ? "default" : "pointer", 'important')
                }}>Następny<FaArrowRight /></Link>
            </div>
        </div>
    )
}
